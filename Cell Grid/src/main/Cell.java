...
public class Cell {
	...
	public void showBorder(Graphics g) {
		if(Main.tileFileNames[Main.map[idxX][idxY]]==Main.tileFileNames[WHITE]) {
			g.drawImage(Main.tiles[BORDER_BLOCK], x-Main.CAMERA_X-(Main.CELL_SIZE>>>3), y-Main.PLAYER_Y-(Main.CELL_SIZE>>>3), Main.CELL_SIZE+(Main.CELL_SIZE>>>2), Main.CELL_SIZE+(Main.CELL_SIZE>>>2), null);
		}
	}
	
	public void show(Graphics g) {
		if(Main.tileFileNames[Main.map[idxX][idxY]]!=Main.tileFileNames[BLACK]) {
			g.drawImage(Main.tiles[Main.map[idxX][idxY]], x-Main.CAMERA_X, y-Main.PLAYER_Y, Main.CELL_SIZE, Main.CELL_SIZE, null);
		}
		...
		/*if(Main.deadly[Main.map[idxX][idxY]]) {
			...
			g.drawRect(x-Main.CAMERA_X, y-Main.PLAYER_Y+(Main.CELL_SIZE>>>1), Main.CELL_SIZE, Main.CELL_SIZE>>>1);
	...
	public void mousePressed(int x, int y, boolean otherBlock) {
		...
		if(x < aparentX)return;
		if(y < aparentY)return;
		if(x >= aparentX + Main.CELL_SIZE)return;
		if(y >= aparentY + Main.CELL_SIZE)return;
